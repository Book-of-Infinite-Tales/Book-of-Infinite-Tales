...
**Loader** (`loader.ts`): parses GitHub URLs/shorthand into a `BookSource`, fetches `book.json`, resolves a separate entries file if needed, then runs validation across the manifest, entries, and structure. Validation errors are surfaced as thrown `Error` messages shown in the UI.

**Encounter picker** (`EncounterPicker.tsx`): When the manifest includes a `structure` block, guides the player through selecting an age, encounter type (character, location, milieu, quest), and sub-selectors that compute a passage id. Without structure, shows a bare passage-number input.
...
The README documents the format from a book-author perspective. The schema in `types.ts` is more expressive than the README's simplified example — the README may lag the schema. The `examples/sample-book/` directory has a working minimal book.
